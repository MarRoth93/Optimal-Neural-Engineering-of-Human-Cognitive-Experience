...
#SBATCH --job-name=06_clipvision_extract
#SBATCH --ntasks=1
...
for sub in "${subjects[@]}"; do
    ...
    python -u /home/rothermm/brain-diffuser/scripts/07_clipvision_extract_features.py --sub "$sub" \
    ...
    if [ "${PIPESTATUS[0]}" -ne 0 ]; then
        echo "!! ERROR: CLIP vision extraction for subject $sub failed"
