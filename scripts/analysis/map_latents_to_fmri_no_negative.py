...
output_base_dir = base_path / "brain_mapping"  # Original outputs
output_base_dir_no_zeros = base_path / "brain_mapping_no_negative"  # No-negative contrast outputs
...
subj_output_dir_no_zeros = output_base_dir_no_zeros / f"subj{sub:02d}"
subj_output_dir.mkdir(parents=True, exist_ok=True)
subj_output_dir_no_zeros.mkdir(parents=True, exist_ok=True)
...
for theta_type in theta_types:
    ...
    for a in alpha_values:
        ...
        fmri_sample = pred_test[target_latent_index]
    ...
    # Create and save no-negative contrasts
    norm_contrast_a = contrast_a_gt_b.get_fdata()
    norm_contrast_a[norm_contrast_a < 0] = 0
    contrast_a_no_neg = nib.Nifti1Image(norm_contrast_a, affine=mask_img.affine, header=mask_img.header)
    contrast_a_file_no_neg = subj_output_dir_no_zeros / f"contrast_alpha-4_gt_alpha4_{theta_type}.nii.gz"
    nib.save(contrast_a_no_neg, contrast_a_file_no_neg)

    norm_contrast_b = contrast_b_gt_a.get_fdata()
    norm_contrast_b[norm_contrast_b < 0] = 0
    contrast_b_no_neg = nib.Nifti1Image(norm_contrast_b, affine=mask_img.affine, header=mask_img.header)
    contrast_b_file_no_neg = subj_output_dir_no_zeros / f"contrast_alpha4_gt_alpha-4_{theta_type}.nii.gz"
    nib.save(contrast_b_no_neg, contrast_b_file_no_neg)

    logging.info(f"Saved no-negative contrast maps ({theta_type}): {contrast_a_file_no_neg}, {contrast_b_file_no_neg}")
