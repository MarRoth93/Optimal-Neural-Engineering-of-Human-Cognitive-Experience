import sys
sys.path.append('/home/rothermm/brain-diffuser/vdvae')  # Add this line
import torch
import numpy as np
...
image_path = '/home/rothermm/brain-diffuser/data/processed_data/subj{:02d}/nsd_train_stim_sub{}.npy'.format(sub,sub)
train_images = batch_generator_external_images(data_path = image_path)

image_path = '/home/rothermm/brain-diffuser/data/processed_data/subj{:02d}/nsd_test_stim_sub{}.npy'.format(sub,sub)
...
for i, x in enumerate(testloader):
    ...
    with torch.no_grad():
        ...
        # Save ref_latent only from first batch
        if not ref_latent_saved:
            ref_latent = stats
            np.savez(
                f"/home/rothermm/brain-diffuser/data/extracted_features/subj{sub:02d}/ref_latents.npz",
                ref_latent=ref_latent
            )
            print(f"ref_latent saved for subject {sub:02d}")
...
np.savez("/home/rothermm/brain-diffuser/data/extracted_features/subj{:02d}/nsd_vdvae_features_31l.npz".format(sub),train_latents=train_latents,test_latents=test_latents)
