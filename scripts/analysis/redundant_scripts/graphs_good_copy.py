...
"""
Minimal script to load model assessor scores and SSIM/PixCorr summaries and generate:
  1) new.png   — 2x1 panels (VDVAE, Versatile); y = (score(α)/score(0)) / (1 − SSIM), α≠0
  ...
  5) scores_averaged_penalized_by_1_minus_pixcorr_models_only_side_by_side.png  — raw/(1−PixCorr)
  6) scores_relative_penalized_by_1_minus_pixcorr_models_only_side_by_side.png — relative/(1−PixCorr)
  7) new_pixcorr.png — 2x1 panels (VDVAE, Versatile); y = (score(α)/score(0)) / (1 − PixCorr), α≠0
"""
...
# Model colors (used in the side-by-side figs)
COLOR_MAP = {
    "VDVAE":               PALETTE[1],  # orange
    "Versatile Diffusion": PALETTE[2],  # green
    "Human Rating":        PALETTE[0],  # blue
}

# --- SINGLE source of truth for task colors (use these in BOTH new() and new_pixcorr()) ---
TASK_COLOR = {
    "emonet": PALETTE[0],  # Valence = blue (#0173b2)
    "memnet": PALETTE[2],  # Memorability = red  (#d55e00)
}
...
def _load_pixcorr_summary_for_subject(subj: int) -> pd.DataFrame:
    ...
    csv_path = PIXCORR_ROOT / f"subj{subj:02d}" / "summary.csv"  # <- subj, not sub
...
# -------- SSIM lookup (from model-level summary_{network}.csv) --------
...
def _build_ssim_lookup():
    """
    ssim_map[network][model][subject][alpha_num] -> ssim_mean
    alpha_num ∈ {-4,-2,2,4}; α=0 treated as 1.0 elsewhere.
    """
...
def plot_scores_averaged_penalized_by_1_minus_pixcorr_models_only(model_data):
    """Penalized raw scores: score(α) / (1 − PixCorr(α, α=0)). α=0 dropped."""
...
def plot_scores_relative_penalized_by_1_minus_pixcorr_models_only(model_data):
    """Relative penalized scores: (score(α)/score(0)) / (1 − PixCorr(α, α=0)). α=0 dropped."""
    pix_map = _build_pixcorr_lookup()
    fig, axs = plt.subplots(1, 2, figsize=(16, 6), sharey=True)
    col_tasks = [("emonet", "Valence"), ("memnet", "Memorability")]
    alpha_no0 = ALPHA_LEVELS_NUM[ALPHA_LEVELS_NUM != 0]
...
def plot_scores_averaged_penalized_by_1_minus_ssim_models_only(model_data):
    """Penalized raw scores: score(α) / (1 − SSIM(α, α=0)). α=0 dropped."""
...
def plot_scores_relative_penalized_by_1_minus_ssim_models_only(model_data):
    """Relative penalized scores: (score(α)/score(0)) / (1 − SSIM(α, α=0)). α=0 dropped."""
    ssim_map = _build_ssim_lookup()
    fig, axs = plt.subplots(1, 2, figsize=(16, 6), sharey=True)
    col_tasks = [("emonet", "Valence"), ("memnet", "Memorability")]
    alpha_no0 = ALPHA_LEVELS_NUM[ALPHA_LEVELS_NUM != 0]
...
def new(model_data):
    ...
    axs[0].set_ylabel("Relative Mean Score / (1 − SSIM)", fontweight="bold")
    axs[1].set_ylabel("Relative Mean Score / (1 − SSIM)", fontweight="bold")
